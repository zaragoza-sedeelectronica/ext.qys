...
import org.sede.servicio.qys.dao.EntidadInternaGenericDAO;
...
public class EntidadInternaController {
	 ...
	 *  Variable servicio
	 */
    public static final String SERVICIO = "entidad-interna";
    /**
	 ...
	 */
    @Autowired
    private EntidadInternaGenericDAO dao;
    ...
    @Description("Listado de entidades gestoras")
    @ResponseClass(value = EntidadInterna.class, entity = SearchResult.class)
    ...
    public @ResponseBody ResponseEntity<?> apiListar(@Fiql SearchFiql search) throws SearchParseException {
    	...
    	return ResponseEntity.ok(dao.searchAndCount(search.getConditions(EntidadInterna.class)));
    }
    ...
    @Permisos(Permisos.DET)
    @Cache(Cache.DURACION_30MIN)
    @Description("Detalle de una entidad gestora")
    @ResponseClass(EntidadInterna.class)
    ...
    public @ResponseBody ResponseEntity<?> apiDetalleEntidad(@PathVariable String id) {
    	EntidadInterna registro = dao.find(id);
    ...
    @ResponseClass(value = EntidadInterna.class)
    public @ResponseBody ResponseEntity<?> apiCrear(@RequestBody EntidadInterna registro) {
    ...
    @ResponseClass(value = EntidadInterna.class)
    public @ResponseBody ResponseEntity<?> apiModificar(@PathVariable String id, @RequestBody EntidadInterna registro) {
