...
if (creando) {
    ...
    if (typeof uriMapa === "undefined") {
        uriMapa = window.location.href.replace('/edit','');
    } else {
        edicion = false;
    }
...
function content(l) {
   if (!l.properties.style) {
    ...
    if (l.properties.updateable === "true") {
        ...
        //console.log('aa');
        //if (l.properties.comments && l.properties.comments.length>0) {
        //    content = content + 'Ver comentarios' + l.properties.comments[0].screen_name;
        //    $("#mensaje").val(l.properties.comments[0].screen_name);
        //}
...
function obtenerJson(propiedades) {
  ...
  map.on('draw:created', function(e) {
        ...
        obj.properties = {}
        obj.properties.id = 'nuevo-' + Date.now();
...
var customControl = L.Control.extend({
  ...
  onAdd: function (map) {
    ...
    //mostrar.style.backgroundImage="url('/cont/paginas/bs/img/material/buscar.png')";
    //mostrar.style.backgroundRepeat='no-repeat';
    //mostrar.innerHTML = '&nbsp;';    
...
function enEdicion() {
    return typeof L.Control.Draw === "function" && edicion == true;
}

origin = '.'
urlAction = './save';
$("#duplicarButton").click(function(event) {
  if (window.confirm("¿Desea duplicar el mapa actual?")) {
    creando = true;
    $("#nombre").val($("#nombre").val() + ' (copia)');
    $("#gczPublicado").val('N');
    urlAction = '../save';
    origin = '..';
  } else {
...
$("#envioGeojson").submit(function(event) {
    $('#mensaje').text('');
    ...
    if (datosGeoJson.pois.length > 0) {
        $.ajax({
            ...
            success: function(data) {
                ...
                if (creando) {
                    window.location.href= origin + '/' + data.id + '/edit'
                }
            },
            fail: function(data) {
                $('#mensaje').text('Error al guardar:' + JSON.stringify(data));
        ...
        $('#mensaje').text('Error al guardar: Debe definir algún recurso asociado al mapa');
        $('#mensaje').addClass('alert alert-error');  
    }
  event.preventDefault();
});